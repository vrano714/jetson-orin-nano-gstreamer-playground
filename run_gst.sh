    ...
    case "$optKey" in
        s)
            if [ "${OPTARG}" = "v4l2" ]; then
                ...
                # need to set x264enc options for better quality?
                SINK="videoconvert ! video/x-raw,format=I420 ! x264enc ! mp4mux ! filesink location=$HOME/hoge.mp4"
            elif [ "${OPTARG}" = "fps" ]; then
                SINK="fpsdisplaysink video-sink=fakesink"
            else
                SINK="autovideosink"
            fi
            ;;
